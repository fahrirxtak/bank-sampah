<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">Edit transaksi operasional</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-8 py-6 border-b border-gray-200">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-xl font-semibold text-gray-800">Form Edit Transaksi</h3>
                            <p class="text-sm text-gray-600">Perbarui data transaksi operasional</p>
                        </div>
                    </div>
                </div>

                <!-- Form Content -->
                <form action="{{ route('admin.transaksi.update', $transaksi->id) }}" method="POST" class="p-8" id="editTransaksiForm">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="relative">
                                    ...
                                    <select id="admin_id"
                                        ...
                                        @foreach($admins as $admin)
                                            <option value="{{ $admin->id }}" {{ old('admin_id', $transaksi->admin_id) == $admin->id ? 'selected' : '' }}>
                                                {{ $admin->name }}
                                            </option>
                                        @endforeach
                                    </select>
    ...
    <script>
        ...
        function resetForm() {
            if (confirm('Apakah Anda yakin ingin mengembalikan data ke nilai semula? Perubahan yang belum disimpan akan hilang.')) {
                // Reset radio buttons
                document.querySelector(`input[name="tipe"][value="{{ $transaksi->tipe }}"]`).checked = true;

                // Reset other fields
                document.getElementById('keterangan').value = "{{ $transaksi->keterangan }}";
                document.getElementById('jumlah').value = "{{ $transaksi->jumlah }}";
                document.getElementById('tanggal').value = "{{ $transaksi->tanggal->format('Y-m-d') }}";
                document.getElementById('admin_id').value = "{{ $transaksi->admin_id }}";
            }
        }
