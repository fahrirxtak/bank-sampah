<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">Ubah data jenis sampah</p>
                </div>
            </div>
    ...
    <div class="py-16">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div
                    class="mb-6 bg-green-100 border border-green-200 text-green-700 px-6 py-4 rounded-lg flex items-center space-x-3 animate-fade-in">
            ...
            @if (session('error'))
                <div
                    class="mb-6 bg-red-100 border border-red-200 text-red-700 px-6 py-4 rounded-lg flex items-center space-x-3 animate-fade-in">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-8 py-6 border-b border-gray-200">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-xl font-semibold text-gray-800">Form Edit Sampah</h3>
                            <p class="text-sm text-gray-600">Ubah data jenis sampah dengan benar</p>
                        </div>
                    </div>
                </div>

                <!-- Form Content -->
                <form action="{{ route('admin.sampah.update', $sampah->id) }}" method="POST" class="p-8"
                    id="editSampahForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="harga_kg" name="harga_kg"
                                        value="{{ old('harga_kg', $sampah->harga_kg) }}" min="0" step="0.01"
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    <label
                                        class="relative flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                        <input type="radio" name="satuan" value="kg" class="sr-only peer"
                                            {{ old('satuan', $sampah->satuan) == 'kg' ? 'checked' : '' }} required>
                                        <div
                                            class="absolute inset-0 border-2 border-transparent peer-checked:border-green-500 peer-checked:bg-green-50 rounded-lg transition-all">
                                        </div>
                                        <div class="relative flex items-center space-x-2">
                                            <div
                                                class="w-4 h-4 bg-green-100 rounded-full flex items-center justify-center peer-checked:bg-green-500">
                                                <i
                                                    class="fas fa-check text-green-600 text-xs peer-checked:text-white"></i>
                                            </div>
                                    ...
                                    <label
                                        class="relative flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                        <input type="radio" name="satuan" value="liter" class="sr-only peer"
                                            {{ old('satuan', $sampah->satuan) == 'liter' ? 'checked' : '' }}>
                                        <div
                                            class="absolute inset-0 border-2 border-transparent peer-checked:border-emerald-500 peer-checked:bg-emerald-50 rounded-lg transition-all">
                                        </div>
                                        <div class="relative flex items-center space-x-2">
                                            <div
                                                class="w-4 h-4 bg-emerald-100 rounded-full flex items-center justify-center peer-checked:bg-emerald-500">
                                                <i
                                                    class="fas fa-check text-emerald-600 text-xs peer-checked:text-white"></i>
                                            </div>
                                    ...
                                    <label
                                        class="relative flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                        <input type="radio" name="satuan" value="buah" class="sr-only peer"
                                            {{ old('satuan', $sampah->satuan) == 'buah' ? 'checked' : '' }}>
                                        <div
                                            class="absolute inset-0 border-2 border-transparent peer-checked:border-lime-500 peer-checked:bg-lime-50 rounded-lg transition-all">
                                        </div>
                                        <div class="relative flex items-center space-x-2">
                                            <div
                                                class="w-4 h-4 bg-lime-100 rounded-full flex items-center justify-center peer-checked:bg-lime-500">
                                                <i
                                                    class="fas fa-check text-lime-600 text-xs peer-checked:text-white"></i>
                                            </div>
    ...
    <script>
        ...
        function resetForm() {
            if (confirm('Apakah Anda yakin ingin mengembalikan nilai awal? Perubahan yang belum disimpan akan hilang.')) {
                // Get original values from server
                const originalValues = @json($sampah);

                // Set form values back to original
                document.getElementById('nama').value = originalValues.nama;
                document.getElementById('harga_kg').value = originalValues.harga_kg;

                // Reset radio buttons
                document.querySelectorAll('input[name="satuan"]').forEach(radio => {
                    radio.checked = (radio.value === originalValues.satuan);
                });
            }
        }
